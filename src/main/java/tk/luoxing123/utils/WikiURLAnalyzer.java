...
public class WikiURLAnalyzer extends Analyzer {
	...
	protected TokenStreamComponents createComponents(String fieldName,
		...
		// result = new WordDelimiterFilter(result, WordDelimiterFilter.DIGIT,
		// null);
	...
	private static class CharacterFilter extends TokenFilter {
		...
		public static void filterChar(final char[] buffer, final int offset,
				final int limit) {
			...
			for (int i = offset; i < limit;) {
				...
				if (currentChars.length == 1
						&& normalizeChars.indexOf(currentChars[0]) >= 0)
